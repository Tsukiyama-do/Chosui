...
func sleepRandom(fromFunction string, ch chan int) {
  //defer cleanup
  defer func() { fmt.Println(fromFunction, "sleepRandom complete due to defer") }()
...
func sleepRandomContext(ctx context.Context, ch chan bool) {
  ...
  defer func() {
    fmt.Println("sleepRandomContext complete due to defer")
    ch <- true
  }()
  ...
  case <-ctx.Done():
    ...
    fmt.Println("sleepRandomContext: Time to return by cancelled from doWorkContext.")
...
func doWorkContext(ctx context.Context) {
  ...
  ctxWithTimeout, cancelFunction := context.WithTimeout(ctx, time.Duration(100)*time.Millisecond)  // grandchild context

  //Cancel to release resources once the function is complete
  defer func() {
    fmt.Println("doWorkContext complete due to defer")
    cancelFunction()
  }()
  ...
  ch := make(chan bool)
  go sleepRandomContext(ctxWithTimeout, ch)    // grand child context
  ...
  case <-ctx.Done():    // self is done, or killed from parent.
    ...
    fmt.Println("doWorkContext: self is stopped from parent context.")
  case <-ch:            // grand child context is returned faster than parent.
    //This case is selected when processing finishes before the context is cancelled
    fmt.Println("sleepRandomContext returned, i.e. grand-child context ended faster.")
  }
}
...
func main() {
  ...
  go func() {
    sleepRandom("Main", nil)   // sleeper 1
    cancelFunction()      // cancel child context
    fmt.Println("Main Sleep complete. canceling context")
  }()
  //Do work
  doWorkContext(ctxWithCancel)     // child context
}
